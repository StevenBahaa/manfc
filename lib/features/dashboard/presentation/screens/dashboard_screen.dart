...
class DashboardScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AppScaffold(
      ...
      child: dashboardAsync.when(
        ...
        data: (data) {
          final totalRevenue = data.invoices.fold<double>(
            0,
            (sum, invoice) => sum + invoice.totalAmount,
          );

          final totalOutstanding = data.invoices.fold<double>(
            0,
            (sum, invoice) => sum + invoice.remainingAmount,
          );

          final totalCollected = data.payments.fold<double>(
            0,
            (sum, payment) => sum + payment.amount,
          );

          final openInvoicesCount = data.invoices
              .where((invoice) => invoice.remainingAmount > 0)
              .length;

          final recentInvoices = [...data.invoices]
            ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
          final displayedInvoices = recentInvoices.take(4).toList();

          final topOutstandingCustomers = _getTopOutstandingCustomers(data.invoices);
            ...
            child: ListView(
              ...
              children: [
                ...
                GridView.count(
                  ...
                  childAspectRatio:
                      AppResponsive.isCompact(context) ? 1.35 : 1.5,
                  children: [
                    _DashboardStatCard(
                  ...
                  child: Column(
                    children: [
                      _SnapshotRow(
                        ...
                        value: CurrencyFormatter.format(totalRevenue, l10n),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      _SnapshotRow(
                        label: l10n.dashboardCollected,
                        value: CurrencyFormatter.format(totalCollected, l10n),
                      ),
                      const SizedBox(height: AppSpacing.md),
                      _SnapshotRow(
                        label: l10n.dashboardOutstanding,
                        value: CurrencyFormatter.format(totalOutstanding, l10n),
                        emphasize: true,
                      ),
                      const SizedBox(height: AppSpacing.md),
                      _SnapshotRow(
                ...
                const SizedBox(height: AppSpacing.md),
                if (topOutstandingCustomers.isEmpty)
                  AppEmptyState(
  ...
  List<_CustomerOutstandingItem> _getTopOutstandingCustomers(
